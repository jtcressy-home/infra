...
- **Cross-platform.** Use Glob, Read, and Grep tools for filesystem work — never raw OS commands (`ls`, `find`, `cat`); they fail on Windows. CLI invocations go through `"$(git rev-parse --show-toplevel)/.codex/gsd-core/bin/gsd_run" intel <subcommand>`, which routes through the Shell Command Projection Module that formats per-OS automatically.
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `"$(git rev-parse --show-toplevel)/.codex/gsd-core/bin/gsd_run" intel extract-exports <file>` to get accurate exports.
...
arch-decisions.json is JSON (NOT markdown). The `"$(git rev-parse --show-toplevel)/.codex/gsd-core/bin/gsd_run" intel` CLI reads, validates, and queries it as JSON. Capture the architecture as descriptive keyed entries:
...
Run: `"$(git rev-parse --show-toplevel)/.codex/gsd-core/bin/gsd_run" intel validate`
...
Run: `"$(git rev-parse --show-toplevel)/.codex/gsd-core/bin/gsd_run" intel snapshot`
