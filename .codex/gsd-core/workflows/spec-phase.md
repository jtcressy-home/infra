...
ones). Reference: @.codex/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / .codex.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  ".codex/gsd-core/bin/lib/edge-probe.cjs" \
  ".codex/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
requirement list). Reference: @.codex/gsd-core/references/prohibition-probe.md — the
...
Use the SPEC.md template from @.codex/gsd-core/templates/spec.md.
