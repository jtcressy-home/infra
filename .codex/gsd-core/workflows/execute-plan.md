...
@.codex/gsd-core/references/git-integration.md
</required_reading>
...
See `.codex/gsd-core/references/tdd.md` for structure.
</tdd_plan_execution>
...
See .codex/gsd-core/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `.codex/gsd-core/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `.codex/gsd-core/templates/summary.md`.
