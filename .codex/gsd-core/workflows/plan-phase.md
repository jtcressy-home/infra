...
@.codex/gsd-core/references/revision-loop.md
@.codex/gsd-core/references/gate-prompts.md
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `.codex/gsd-core/references/skeleton-template.md` — the planner reads it when producing SKELETON.md (lazy; not loaded on non-skeleton runs).
...
**If found:**
1. Read template: `.codex/gsd-core/templates/VALIDATION.md`
...
**Read and run** the gate + edge probe in `.codex/gsd-core/references/specless-probe-fallback.md`
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `.codex/gsd-core/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
**WALKING_SKELETON:** ${WALKING_SKELETON} (when true, the first deliverable must be a Walking Skeleton — Read the template at `.codex/gsd-core/references/skeleton-template.md` and produce SKELETON.md alongside PLAN.md.)
**Granularity:** {granularity}

${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Read `.codex/gsd-core/references/planner-mvp-mode.md` now and follow its vertical-slice planning rules. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
...
else-branch below, per §A/§B/§C of `.codex/gsd-core/references/specless-probe-fallback.md`
