...
```bash
PHASE_STATE=$("$(git rev-parse --show-toplevel)/.codex/gsd-core/bin/gsd_run" query init.phase-op ${PHASE_NUM})
```
...
```bash
DETAIL=$("$(git rev-parse --show-toplevel)/.codex/gsd-core/bin/gsd_run" query roadmap.get-phase ${PHASE_NUM})
```
...
```bash
"$(git rev-parse --show-toplevel)/.codex/gsd-core/bin/gsd_run" query commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
