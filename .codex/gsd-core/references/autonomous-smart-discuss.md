...
```bash
PHASE_STATE=$(node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" query init.phase-op ${PHASE_NUM})
```
...
```bash
DETAIL=$(node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" query roadmap.get-phase ${PHASE_NUM})
```
...
```bash
node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" query commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
